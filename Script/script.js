...
function loadCollapse(sortType) {
  ...
  for (let index = 1; index < temp.length - 1; index++) {
    ...
    ).innerHTML = `<img alt="syringe" src="Images/syringe.png" style="height:14px;"> ${formatNumberWithCommas(
      data.totalVaccine
    )}`;
    ...
    CONTENT.querySelector(
      ".popolationpercent"
    ).innerHTML = `<img alt="syringe" src="Images/Population.png" style="height:14px;"> ${data.vaccinatedPercent}`;
    CONTENT.querySelector(
      ".differenceCounter"
    ).innerHTML = `+${formatNumberWithCommas(data.totalVaccineDifferences)}`;

    // Set attribute
    CONTENT.querySelector(".accordion").setAttribute(
      "data-country-name",
      data.name
    );
...
function UpdateListFilter(selectedItem) {
  switch (selectedItem) {
    case 0:
      ...
      $("#collapseCountries").append(
        '<div class="col-12 groupList-title">Countries</div>'
      );
      $("#collapseContinents").empty();
      $("#collapseContinents").append(
        '<div class="col-12 groupList-title">Continents</div>'
      );
      loadCollapse(0);
      loadContinents(0);
      $("#dropdownMenuButton").html(
        '<img alt="syringe" src="Images/syringe.png" style="width: 25px; padding-right:10px;">Doses Administered'
      );
      break;
    case 1:
      $("#collapseCountries").empty();
      ...
      loadContinents(1);
      $("#dropdownMenuButton").html(
        '<img alt="population" src="Images/Population.png" style="width: 25px; padding-right:10px;">% fully vaccinated'
      );
      break;
  }
}
...
function showCountryData(countryname) {
  ...
  window.location.href =
    "View/country.html" + "?" + countryname.replace(" ", "");
}
