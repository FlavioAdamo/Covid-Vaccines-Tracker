...
async function GetCountryData(country_name) {
  ...
  return allData.filter(
    (word) =>
      word.split(",")[0] == country_name.toString().replaceAll("%20", " ")
  );
}
...
function GetContinentsData() {
  ...
  continents.push(
    lastDataAvaible.filter((word) => word.split(",")[0] == "Europe").toString()
  );
  continents.push(
    lastDataAvaible.filter((word) => word.split(",")[0] == "Asia").toString()
  );
  continents.push(
    lastDataAvaible
      .filter((word) => word.split(",")[0] == "North America")
      .toString()
  );
  continents.push(
    lastDataAvaible.filter((word) => word.split(",")[0] == "Africa").toString()
  );
  continents.push(
    lastDataAvaible
      .filter((word) => word.split(",")[0] == "South America")
      .toString()
  );
  continents.push(
    lastDataAvaible.filter((word) => word.split(",")[0] == "Oceania").toString()
  );
  return continents;
}
...
async function GetCountriesByIncome() {
  ...
  continents.push(
    lastDataAvaible.filter((word) => word.split(",")[0] == "High income")
  );
  continents.push(
    lastDataAvaible.filter(
      (word) => word.split(",")[0] == "Upper middle income"
    )
  );
  continents.push(
    lastDataAvaible.filter((word) => word.split(",")[0] == "Low middle income")
  );
  return incomeData;
}
